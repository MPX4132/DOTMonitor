...
function Icon:Masked(masked)
	self.isMasked = (masked == nil) or (masked ~= false)
	self.textureMask[self.isMasked and "Show" or "Hide"](self.textureMask)
end
...
function Icon:SetBackground(texturePath)
	...
	if not self.texture then
		...
		self.texture:SetDrawLayer("ARTWORK", 7) -- Default Draw Layer (0)
...
function Icon:SetBackgroundMask(maskTexturePath, maskBlendMode)
	if not self.textureMask then
		self.textureMask = self:CreateTexture()
		self.textureMask:SetDrawLayer("ARTWORK", 0) -- Highest Layer of artwork
		self.textureMask:SetAllPoints(self)
	end
	self.textureMask:SetBlendMode(maskBlendMode)
	self.textureMask:SetTexture(maskTexturePath)
end
...
function Icon:New(ID, backgroundPath, parentFrame)
	...
	icon.SetBackgroundMask = Icon.SetBackgroundMask
